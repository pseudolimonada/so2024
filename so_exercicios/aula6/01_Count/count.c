...
void *increment_count(void *t)
{
  ...
  /**** BEGIN SOLUTION ****/
//quando threads mudam as condicoes (exited_threads neste caso) e boa pratica so dar unlock do mutex depois de mandar o sinal, pq podem estar outras threads a usar a mesma condicao
  pthread_mutex_lock(&count_mutex);
  exited_threads++;
...
int main (int argc, char *argv[])
{
  ...
  for(i = 1; i < NUM_WORKER_THREADS+1; i++){
	id[i] = i;
        pthread_create(&threads[i], NULL, increment_count, &id[1]);
